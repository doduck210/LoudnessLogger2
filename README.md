...
- 파일 이름과 시간 단위 분할 방식은 기존 logger와 호환된다.
  - `YYYY-MM-DD/YYYY-MM-DD_HH.00.00.wav` (정시에 시작된 segment)
...
각 recorder의 `--output`은 포트의 기본 디렉터리를 지정한다. 예를 들어
`--output /mnt/hdd/recordings/decklink2`로 실행하면 실제 파일은 다음처럼 KST 날짜별
하위 디렉터리에 저장된다.
...
└── decklink2/
    ├── 2026-08-11/
    │   ├── 2026-08-11_14.00.00.wav
    │   └── 2026-08-11_14.00.00_mlkfs.csv
    └── 2026-08-12/
        └── ...
```

리포트와 웹 서버 설정에는 날짜 폴더가 아니라 기존처럼 포트 기본 디렉터리
`/mnt/hdd/recordings/decklink2`를 지정한다. 프로그램이 날짜 폴더를 자동으로 찾는다.
...
```text
2026-07-22/2026-07-22_12.00.00_mlkfs.csv
...
편성 오디오 추출기는 WAV 파일명의 서버 시작시각과 WAV의 실제 전체 sample 수를
사용한다. 연속된 다음 WAV의 파일명을 정확한 종료 경계로 사용하므로 시간당 sample
수가 조금 달라도 파일 사이에 틈이 생기지 않는다.
...
리포트 XLSX, 레코더 로그와 리포트 로그의 보관기간을 각각 0~3650일로 지정한다. 웹 운영
...
삭제하지 않는다.
매일 설정된 예약 시각에 설정 일수보다 오래된 방송일 자료를 정리한 뒤, 자동
리포트가 활성화되어 있으면 전날 리포트를 생성한다. 실행 중인 리포트·오디오 작업 날짜와
...
KST 날짜가 바뀌면 전날 내용을 날짜가 붙은 `.gz` 파일로 압축한 뒤 원본 로그를
비우고, 설정한 기간보다 오래된 압축 로그만 삭제한다. 리포트와 편성 오디오의
상세 출력은 `logs/reports/{채널ID}/YYYY-MM-DD.log`에 계속 추가된다. 웹의
**로그** 화면에서 채널·날짜·종류를 선택해 현재 및 과거 로그를 볼 수 있다.
`settings.json`과
...
행의 **듣기** 버튼으로 큰 전용 재생 창을 열 수 있다. 이 창에서 재생 위치와
볼륨을 조절하고 10초씩 앞뒤로 이동하거나 재생속도를 바꿀 수 있으며, WAV 원본도
다운로드할 수 있다.

캘린더의 **기간 자료 한 번에 받기**에서는 채널과 시작일·종료일을 선택해 최대
366일치 리포트 XLSX와 시간별 M-LKFS CSV를 ZIP 하나로 다운로드한다. 기본 범위는
전날까지 최근 90일이며 종료일은 서울 기준 전일 이후로 지정할 수 없다.
ZIP 안은 `reports/`와 `mlkfs/YYYY-MM-DD/`로
나뉘며 `manifest.json`에서 날짜별 파일 수와 누락 여부를
확인할 수 있다. 서버는 ZIP64 형식으로 압축 데이터를 바로 전송하므로 장기간
자료도 별도의 임시 복사본을 만들지 않는다.

선택한 종료일의 저장 편성표를 확인해 마지막 편성 종료시각이 다음 날 새벽이면,
그 종료시각이 포함된 시간대까지 다음 날 M-LKFS CSV를 자동으로 추가한다.
종료일 편성표가 없거나 읽을 수 없으면 자정까지만 포함하고 그 사유를
`manifest.json`의 `mlkfsCoverage.warning`에 기록한다.
