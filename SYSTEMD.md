...
**리포트 로그** 보관기간과 공통 **웹 운영 로그** 보관기간을 사용한다. 웹 서버가
KST 날짜 변경을 감지하면 현재 `logs/*.log`를 전날 날짜의 `.gz` 파일로 압축하고
원본 로그를 비운다. 리포트와 편성 오디오 출력은
`logs/reports/{채널ID}/YYYY-MM-DD.log`에 저장된다. 설정 기간보다 오래된 로그는
매일 정리 시각에 삭제하며, 웹의 **로그** 화면에서도 날짜별로 조회할 수 있다.
